...
VariSel <- R6Class("VariSel",
                  ...
                  public = list(
                    ...
                    stab = NULL,
                    univ = NULL,
                    initialize = function(x, y, univ = TRUE,  Sigma_12inv = diag(1, ncol(as.data.frame(y))), sepy ="__"){
                      self$univ <- univ
                      ...
                      if(!univ){
                        ...
                        private$y <- as.data.frame(as.numeric(as.matrix(private$y) %*% Sigma_12inv))
                        colnames(private$y) <- "allresponses"
                        Eg <- expand.grid(colnames(x), colnames(y))
                        private$x <- kronecker(Matrix::t(Sigma_12inv), private$x)
                        colnames(private$x) <- paste(Eg[,2],Eg[,1], sep = sepy)
                      }
                    ...
                    estime = function(){
                      self$res <- self$res %>%
                        mutate(Yhat = map2(Beta, Intercept, ~ cbind(1,private$x) %*%
                               ...
                               BIC = map2(MSE, Df,~ private$n*log(.x / private$n) + log(private$n) * .y))
                    },
                    ...
                    plot_error = function(print = TRUE){
                     ...
                     self$res$MSE %>%
                        map( function(x){x %>% gather() %>% mutate(key = as.numeric(gsub('s', '', key))) %>%
