...
VariSel <- R6Class("VariSel",
 ...
 public = list(
  ...
  initialize = function(x, y, univ = TRUE,
                 Sigma_12inv = diag(1, ncol(as.data.frame(y))), sepy ="__"){
    self$univ <- univ
    ...
    if (!univ){
      ...
      private$y <- as.numeric(as.matrix(private$y) %*% Sigma_12inv) %>%
        as.data.frame()
 ...
 self$res %>%
  mutate( Beta = map(Beta, ~rowid_to_column(.) %>%
            gather(key, value, -rowid) %>% as.tibble()),
          Beta = map2(Beta, Trait,
                      ~ mutate(.x, key = as.numeric(gsub("s", "", key)),
                         Trait = .y, pos = name_pos[rowid]))) %>%
  select(Beta) %>%
