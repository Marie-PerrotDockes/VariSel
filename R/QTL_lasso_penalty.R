  ...
  public = list(
      ...
      mutate(Data = map(Data, ~.$value),
       Model = purrr::map(Data,
        ~ cv.glmnet(private$x, .,
          ...
          intercept = intercept)),
        Penalty_factor = map(Model ~ (coef(.)[-1] + 1 / sqrt(nrow(private$x))^(-1))),
        Model = map2(Model, Penalty_factor,
            ~ glmnet(private$x, .x,
              penalty.factor = .y,
              lambda = lambda,
              standardize = standardize,
    ...
    sel_cv = function(s = "lambda.min"){
     if(is.null(self$cv) || (s != private$s)){
      ...
      nest(.key = Data) %>%
      mutate(Data = map(Data, ~.$value),
             Model = map(Data,~cv.glmnet(private$x, .)),
             Penalty_factor = map(Model, ~ (coef(.)[-1] + 1 / sqrt(nrow(private$x))^(-1))),
             Model = map2(Data, Penalty_factor,
                          ~cv.glmnet(private$x, .x,
                             penalty.factor = .y))
             )
      }
      self$cv <- self$cv %>%
